...
config = defaultConfiguration
    { deployCommand = "git stash && git checkout develop && stack exec josietj clean && stack exec josietj build && git fetch origin && git checkout -b master --track origin/master && cp -a _site/. . && git add -A && git commit -m \"Publish.\" && git push origin master:master && git checkout develop && git branch -D master && git stash pop"
    }
